    ...
    def forward(self, voxels, points=None, junction_points=None, endpoint_points=None, mode='train'):
        ...
        if mode == 'train':
            if points is not None:
                _, G = self.generator(points, z, junction_points, endpoint_points)
                return G
            else:
...
class BAE_NET_Wrapper:
    ...
    def _load_data(self):
        ...
        for idx, npz in enumerate(files):
            data_npz_name = os.path.join(self.data_dir, npz)
            try:
                data = np.load(data_npz_name)

                # --- 1. 读取基础数据 ---
                vox = data['voxels']
                pts = data['sdf_points']  # 这里读的是全部点，用于训练
                ...
                if 'occu_values' in data:
                    ...
                    print(f"⚠️ {npz}: 缺少 occu_values/sdf_values，跳过")
                    continue

                # --- 2. 读取骨骼 ---
                # 使用字典安全读取
                keys = list(data.files)
                j_pts = np.asarray(data["junction_points"],
                                   dtype=np.float32) if "junction_points" in keys else np.zeros((0, 3),
                                                                                                dtype=np.float32)
                e_pts = np.asarray(data["endpoint_points"],
                                   dtype=np.float32) if "endpoint_points" in keys else np.zeros((0, 3),
                                                                                                dtype=np.float32)

                # --- 3. [DEBUG] 诊断原始数据 ---
                if idx < 3:  # 只打印前3个文件的详细信息，避免刷屏
                    print(f"\n📄 文件: {npz}")
                    # 过滤内部点来计算物体真实范围
                    inside_pts = pts[occ > 0.5]
                    if len(inside_pts) == 0:
                        print("  ⚠️ 警告: 该物体没有内部点 (Occupancy全为0?)")
                        c_scale = 1.0  # 默认防止除以0
                        c_center = np.zeros(3)
                    else:
                    ...
                    # 合并骨骼看范围
                    all_skel = []
                    if len(j_pts) > 0: all_skel.append(j_pts)
                    if len(e_pts) > 0: all_skel.append(e_pts)

                    if len(all_skel) > 0:
                        skel_cat = np.concatenate(all_skel, axis=0)
                        s_min, s_max = np.min(skel_cat, axis=0), np.max(skel_cat, axis=0)
                        s_scale = np.max(s_max - s_min)
                        s_center = (s_min + s_max) / 2.0
                        print(f"  🔸 骨骼(原始):   尺度={s_scale:.4f}, 中心={s_center}")
                        print(f"     原始数据举例: {skel_cat[0]}")

                        # 判断倍数
                        ratio = s_scale / c_scale if c_scale > 0 else 0
                        print(f"  ⚖️ 尺度比例 (骨骼/物体): {ratio:.2f}倍")
                    else:
                        print("  🔸 骨骼: 无有效点")
                ...
                if len(inside_pts) > 0:
                    ...
                    if len(all_skel) > 0:
                        ...
                        # 如果骨骼比物体大 1.5 倍以上，启动缩放
                        if s_scale > c_scale * 1.5:
                            scale_factor = c_scale / s_scale
                            if idx < 3: print(f"  🛠️  检测到尺度不匹配，执行缩放 (Factor={scale_factor:.4f})")

                            # 应用缩放
                            if len(j_pts) > 0:
                                j_pts = (j_pts - s_center) * scale_factor + c_center
                            if len(e_pts) > 0:
                                e_pts = (e_pts - s_center) * scale_factor + c_center
                        elif idx < 3:
                            print("  ✅ 尺度匹配，无需缩放")

                # 5. 存入列表
                voxels_list.append(vox)
                points_list.append(pts)  # 存全部点！不要只存内部点！
        ...
        for i, pts in enumerate(junction_list):
            if len(pts) > 0: self.data_junctions[i, :len(pts), :] = pts
        for i, pts in enumerate(endpoint_list):
            if len(pts) > 0: self.data_endpoints[i, :len(pts), :] = pts
    ...
    def generate_mesh(self, voxels, junction_points=None, endpoint_points=None, threshold=0.5):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(grid_points), batch_size):
                ...
                # Pass skeleton points
                branch_pred, pred = self.model.generator(
                    batch_tensor,
                    z,
                    junction_points=j_tensor,
                    endpoint_points=e_tensor
                ...
                predictions.append(branch_pred.squeeze(0).cpu().numpy())
                total_mesh.append(pred.squeeze(0).cpu().numpy())
